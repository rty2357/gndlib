/*
 * gnd-lkf.hpp (gnd likelihood field)
 *
...
#define GND_DEBUG_LOG_NAMESPACE2 lkf
...
namespace gnd {
	namespace lkf {
		 ...
		 */
		static const char CMapFileNameDefault[] = "lkf";
		...
		struct counting_map_pixel {
			...
			vector::fixed_column< PosDim > pos_sum;
		...
		struct map_pixel {
			/// @brief mean
			vector::fixed_column< PosDim > mean;
		 ...
		 * @see likelihoodfield
		 */
		typedef struct likelihoodfield map_t;
...
namespace gnd {
	namespace lkf {
		...
		int build_map(map_t *map, cmap_t *cnt, double sr = 20, uint32_t alpha = 100, double blur = ErrorMargin, uint64_t np = 10 );
...
namespace gnd {
	namespace lkf {
		...
		int clear_counting_map(cmap_t *m) {
			...
			{ // ---> operation
				lkf::counting_map_pixel ini;

				for( size_t i = 0; i < lkf::PlaneNum; i++){
		...
		int counting_map(cmap_t *m, double x, double y)
		{
			...
			{ // ---> operate
				...
				for(size_t i = 0; i < PlaneNum; i++){
					...
					vector::fixed_column<PosDim> 	core;	// pixel core position
					vector::fixed_column<PosDim> 	pos;	// reflection point on pixel
...
//					} // <--- obtain the number of points
		 ...
		 */
		inline
		int build_map(map_t *map, cmap_t *cnt, double sr, uint32_t alpha, double blur, uint64_t np ) {
...
public:
	/// map type
	typedef lkf::map_t					map_t;
	/// map type pointer
	typedef map_t*						map_pt;
	/// pixel type
	typedef lkf::pixel_t 				pixel_t;
	/// pixel type pointer
